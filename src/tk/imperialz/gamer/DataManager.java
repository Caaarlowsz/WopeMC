...
public class DataManager {
	...
	public static void createPlayer(final UUID uuid, final String nick) {
		try {
			MySQL.con.createStatement()
					.executeUpdate("INSERT INTO `KitPvP` (`uuid`, `kills`, `deaths`, `nick`) VALUES ('" + uuid
							+ "', '0', '0', '" + nick + "')");
	...
	public static void updateData(final PlayerData playerData) {
		try {
			MySQL.con.createStatement()
					.executeUpdate("UPDATE `KitPvP` SET `Kills`='" + playerData.getKills() + "', `NICK`='"
							+ playerData.getNick() + "', `Deaths`='" + playerData.getDeaths() + "' WHERE `uuid`='"
							+ playerData.getUUID() + "';");
		} catch (SQLException e) {
			e.printStackTrace();
		}
