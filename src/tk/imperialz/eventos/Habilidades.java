...
public class Habilidades implements Listener
{
    ...
    public void onRemoverTNT(final InventoryClickEvent e) {
        ...
        if (KitAPI.getkit(p) != "Nenhum" && e.getSlot() == 39 && e.getCurrentItem().getType().equals((Object)Material.TNT) && Habilidades.emhotpotato.contains(p.getName())) {
    ...
    public void aoinv(final InventoryClickEvent e) {
        try {
            final Player p = (Player)e.getWhoClicked();
            if (Habilidades.emphantom.contains(p.getName()) && e.getSlotType().equals((Object)InventoryType.SlotType.ARMOR)) {
                e.setCancelled(true);
            }
        }
        catch (Exception ex) {}
    ...
    public void monkNeero(final PlayerInteractEntityEvent event) {
        ...
        if (KitAPI.getkit(Player2) == "Monk" && Player2.getItemInHand().getType() == Material.BLAZE_ROD) {
            ...
            if (lastUsed + 15000L > System.currentTimeMillis()) {
                event.getPlayer().sendMessage(String.format("§cVoc\u00ea est\u00e1 em cooldown de ", -(System.currentTimeMillis() - (lastUsed + 15000L)) / 1000L));
            }
            else {
        ...
        if (event.getRightClicked() instanceof Player) {
            ...
            if (p.getItemInHand().getType() == Material.IRON_FENCE && KitAPI.getkit(p) == "Gladiator") {
                ...
                if (this.generateGlass) {
                    ...
                    Habilidades.id2 = Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getPlugin(), (Runnable)new Runnable() {
                        @Override
                        public void run() {
                            if (Habilidades.fighting.containsKey(p.getName()) && Habilidades.fighting.get(p.getName()).equalsIgnoreCase(r.getName()) && Habilidades.fighting.containsKey(r.getName())) {
                                Habilidades.fighting.get(r.getName()).equalsIgnoreCase(p.getName());
                            }
                        }
                    }, 2400L);
                    Habilidades.id1 = Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getPlugin(), (Runnable)new Runnable() {
                        @Override
                        public void run() {
                ...
                if (k.getItemInHand().getType() == Material.WOOD_SWORD) {
                    ...
                    if (k.getItemInHand().containsEnchantment(Enchantment.DAMAGE_ALL)) {
                        ...
                        if (lvl + 1 <= 10) {
                            ...
                            k.sendMessage("§a Ops, voc\u00ea acabou atingindo o n\u00edvel m\u00e1ximo! N\u00e3o \u00e9 mais poss\u00edvel upar!");
                        }
                    }
                    else {
                        k.getItemInHand().addEnchantment(Enchantment.DAMAGE_ALL, 1);
